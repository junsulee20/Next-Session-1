...
export default function BooksPage() {
  ...
  function startEdit(book) {
    ...
    setEditingPageCount(book.pageCount?.toString() || "");
  }
  ...
  async function saveEdit(id) {
    ...
    try {
      const body = {
        ...
        pageCount: parseInt(editingPageCount) || 0,
        excerpt: "",
        publishDate: new Date().toISOString(),
      };
          ...
          <li key={book.id} style={{ border: "1px solid #ddd", padding: 12, borderRadius: 8 }}>
            {editingId === book.id ? (
              ...
              <div style={{ display: "grid", gap: 4 }}>
                ...
                {book.description && (
                  <div style={{ fontSize: 14, color: "#555" }}>{book.description}</div>
                )}
                <div style={{ fontSize: 12, color: "#666" }}>id: {book.id}</div>
                {book.pageCount && (
                  <div style={{ fontSize: 12, color: "#666" }}>페이지 수: {book.pageCount}</div>
                )}
